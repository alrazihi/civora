...
func TestCaseRepository_SaveAndFind(t *testing.T) {
	...
	c, err := domain.NewCase(orgID, userID, "Emergency Food Request", "Family needs emergency food assistance", "GENERAL", "NORMAL", nil)
...
func TestCaseRepository_TenantIsolation(t *testing.T) {
	...
	c, err := domain.NewCase(org1, user1, "Case in Org 1", "Description", "GENERAL", "NORMAL", nil)
	require.NoError(t, err)
	err = repo.Save(context.Background(), c)
	require.NoError(t, err)
...
func TestCaseRepository_UpdateStatus(t *testing.T) {
	...
	c, err := domain.NewCase(orgID, userID, "Test Case", "Description", "GENERAL", "NORMAL", nil)
	require.NoError(t, err)
	err = repo.Save(context.Background(), c)
	require.NoError(t, err)
...
func TestCaseRepository_FindByOrganization(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		c, err := domain.NewCase(orgID, userID, "Case", "Description", "GENERAL", "NORMAL", nil)
		require.NoError(t, err)
		err = repo.Save(context.Background(), c)
		require.NoError(t, err)
	}
...
func TestCaseRepository_Assign(t *testing.T) {
	...
	c, err := domain.NewCase(orgID, creatorID, "Test Case", "Description", "GENERAL", "NORMAL", nil)
	require.NoError(t, err)
	err = repo.Save(context.Background(), c)
	require.NoError(t, err)
...
func TestCaseRepository_ForeignKeyConstraint(t *testing.T) {
	...
	c, err := domain.NewCase(fakeOrgID, fakeUserID, "Test Case", "Description", "GENERAL", "NORMAL", nil)
	require.NoError(t, err)
	err = repo.Save(context.Background(), c)
...
func TestCaseRepository_CaseNumberCollisionRetries(t *testing.T) {
	...
	_, err := db.ExecContext(context.Background(), directQuery,
		...
		domain.CaseStatusNew, "GENERAL", "NORMAL", nil, userID, nil,
		time.Now().UTC(), time.Now().UTC(), nil)
	require.NoError(t, err)

	c, err := domain.NewCase(orgID, userID, "Collision Case", "Description", "GENERAL", "NORMAL", nil)
	require.NoError(t, err)
